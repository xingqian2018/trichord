# Method D — N Node, M Process, Batch Process, Result Gather

An example of a nN_mP (i.e. N Node, M Process (i.e. 1 Process for 1 GPU) ) batch process code, then gather results
...
def main() -> None:
    ...
    result_buffer_sharded = <list or dict depend on need>
    ...
    while True:
        ...
        for si, ri in zip(samples_batch, response_list):
            if <is_valid(ri)>:
                ...
                <store_result(all_valid_info -> result_buffer_sharded)>
            else:
                si['try_num'] += 1
                samples_error.append(si)
    ...
    result_buffer_gathered = distributed.gather_object(result_buffer_sharded)

    if rank == 0:
        result_final = <convert_to_final(result_buffer_gathered)>
        <save_or_display(result_buffer_gathered)>
